...
class optimization():
    ...
    def set_hyperparameters(self):
        ...
        self.space = [
                    ...
                    Integer(100, 2000, name='n_estimators'), #100, 800, 1200
    ...
    def run(self, optimization_method, n_calls=100):
        ...
        elif self.optimization_method == 'gradient_boosted_trees':
        ...
        self.opt = opt_method(
                            ...
                            n_calls = n_calls,
                            # n_jobs = 1, # force to use 1 core to avoid nested parallelism
                            verbose = -1
                            ) 
        ...
        opt_hyperparameters= pd.DataFrame(
                                            [self.values],
                                            columns=self.hyperparameters                   
                                         )
        # Convert np.int64 values to regular integers
        opt_hyperparameters = opt_hyperparameters.applymap(lambda x: int(x) if isinstance(x, np.int64) else x)

        joblib.dump(opt_method, os.path.join(self.path_to_save, r'optimization_method_' + self.target + '.pkl'))
        print('Saved:', 'optimization_method_' + self.target + '.pkl')

        opt_hyperparameters.to_pickle(os.path.join(self.path_to_save, r'optimization_data_' + self.target + '.pkl'))
    ...
    def convergence_plot(self):
        ...
        print('Saved:', 'optimization_result_' + self.target + '.png')
    ...
    def train_model(self):
        ...
        #deprecated function:
        #self.model.set_config(verbosity=-1)

        self.model.fit(self.sample_train[self.RFE_features], 
    ...
    def confusion_matrix(self, target_names):
        ...
        # Pretty confusion matrix (Upgraded to ConfusionMatrixDisplay Module) 
    ...
    def evaluate(self, strategy, *args, **kwargs):
        ...
        if self.problem == 'classification':
            ...
            print('RMSE: ', np.sqrt(mean_squared_error(X, Y)))# mean_squared_error(X, Y, squared=False))
    ...
    def ROC(self, overall_performance, *args, **kwargs):
        ...
        if self.n_classes == 2:
            ...
            display = RocCurveDisplay(fpr=self.fpr, tpr=self.tpr).plot(ax=ax, label='Receiver Operating Characteristic')
        ...
        print('Average ROC AUC score, micro-averaged over all classes: {0:0.10f}'
                    .format(roc_auc_score(np.array(self.y_test_2.tolist()), self.y_pred_2, average='micro')))

        print('Average ROC AUC score, macro-averaged over all classes: {0:0.10f}'
                    .format(roc_auc_score(np.array(self.y_test_2.tolist()), self.y_pred_2, average='macro')))

        print('Average ROC AUC score, weighted-averaged over all classes: {0:0.10f}'
                    .format(roc_auc_score(np.array(self.y_test_2.tolist()), self.y_pred_2, average='weighted')))
    ...
    def DET(self, *args, **kwargs):
        ...
        if self.n_classes == 2:
            ...
            plt.plot(
                    ...
                    linestyle='-',
                    linewidth=linewidth,
                    label='Detection Error Tradeoff'
                    )
        ...
        plt.tick_params(axis='both', which='major', labelsize=fontsize, direction='in')
        plt.legend(loc="upper left", fontsize=fontsize, framealpha=1)
    ...
    def PR(self, *args, **kwargs):
        ...
        if self.n_classes == 2:
            ...
            pr_display = PrecisionRecallDisplay(precision=prec, recall=recall).plot(ax=ax,label = 'Precision-Recall')
            ax.yaxis.get_major_ticks()[0].label1.set_visible(False)
        ...
        lw=2
        labelsize = 18
        plt.plot([0, 1], [0.5, 0.5], 'k--', lw=lw, label = 'Random Classifier')
